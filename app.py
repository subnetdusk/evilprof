...
from config import (
    DEFAULT_NUM_TESTS, EXAMPLE_IMAGE_PATH, ANALYSIS_IMAGE_PATH # Importa entrambe le costanti immagine
)
...
with st.expander(T("INSTRUCTIONS_HEADER"), expanded=False):
    ...
    # Usa colonne per centrare e creare margini
    # Use columns to center and create margins
    img_col1, img_col2, img_col3 = st.columns([0.15, 0.7, 0.15]) # 15% margine sx, 70% img, 15% margine dx
    with img_col2: # Mette le immagini nella colonna centrale
        try:
            # Prima immagine: Esempio Excel
        ...
        st.markdown("---") # Separatore tra le immagini

        try:
            # Seconda immagine: Analisi Statistica
...
if uploaded_file is not None:
    ...
    if st.session_state.processed_filename != uploaded_file.name:
        ...
        if error_k:
            ...
            # sidebar_status_placeholder.success(F("FH_LOAD_COMPLETE_BLOCKS", count=len(all_q), num_blocks=len(blocks_sum))) # Silenziato
...
if st.session_state.blocks_summary:
    ...
    for block_info in st.session_state.blocks_summary:
        block_id = block_info['block_id']; block_type_str = block_info['type']
        ...
        st.session_state.block_requests[block_id] = st.sidebar.number_input(
            ...
            value=value_to_set, step=1, key=f"block_input_{block_id}"
        )
...
if validation_button:
    ...
    with output_placeholder:
        if test_results is not None:
            ...
            if not test_results and not excel_file_created: st.warning(T("VALIDATION_NO_MESSAGES"))
            elif test_results:
...
if generate_button:
    ...
    if uploaded_file is None: output_placeholder.warning(T("UPLOAD_FIRST_WARNING")); st.stop()
    ...
    if total_requested <= 0: output_placeholder.error(T("TOTAL_QUESTIONS_ZERO_ERROR_BLOCKS")); st.stop()
    ...
    with output_placeholder:
        ...
        elif not final_generation_messages: st.error(T("PDF_GENERATION_ERROR"))
