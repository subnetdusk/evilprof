# test.py (New statistical analysis with 15 tests per sequence)
import pandas as pd
import random
import os
import math
...
OUTPUT_EXCEL_FILE = "similarity_analysis_k_per_block_dice_mc_15t.xlsx" # Aggiornato nome output / Updated output name
...
def _run_single_analysis_k_per_block(k_per_block, blocks_info, all_questions_list, num_tests_to_generate):
    ...
    """
    dice_by_distance = {}
    # La distanza massima ora sarà num_tests_to_generate - 1 (cioè 14 se num_tests=15)
    # Max distance will now be num_tests_to_generate - 1 (i.e., 14 if num_tests=15)
...
def run_all_tests(status_callback, num_monte_carlo_runs=30):
    ...
    # --- MODIFICA QUI ---
    num_tests_per_sequence = 15 # Numero di test consecutivi da generare / Number of consecutive tests
    # --- FINE MODIFICA ---
    k_per_block_values = range(1, 7) # Da 1 a 6 / From 1 to 6
    max_distance_overall = 0
    progress_update_frequency = 5

    monte_carlo_summary.append(("info", "MC_TEST_KPB_STARTING", {"num_runs": num_monte_carlo_runs, "num_k": len(k_per_block_values), "k_range": f"1-{max(k_per_block_values)}", "num_tests": num_tests_per_sequence}))
    ...
    for run in range(1, num_monte_carlo_runs + 1):
        ...
        for k_block in k_per_block_values:
            ...
            if avg_dice_by_distance is not None:
                ...
                monte_carlo_summary.append(("warning", "MC_TEST_FAILED_FOR_KPB_IN_RUN", {"k_per_block": k_block, "run": run}))
